...
class DataModel
{
    ...
    static getAllData()
    {
        return data.query(`SELECT * FROM account AS a
            ...
            WHERE a.active = 1 AND m.active = 1 AND v.active = 1`, null, done);
    };
    ...
    static getAllDataByAccountId(id)
    {
        return data.query(`SELECT * FROM account AS a
            ...
            INNER JOIN version AS v ON v.module_id = m.id
            WHERE a.id = ${id} 
            AND a.active = 1 AND m.active = 1 AND v.active = 1`, null, done);
    };
    ...
    static insertModule(name, email, done)
    {
        data.query(`SELECT id FROM account WHERE email = '${email}'`, null, (accounts) => {
            ...
            data.query(`INSERT INTO module (name, account_id, active) VALUES ('${name}', '${account_id}', 1`, null, done);
        });
    };
