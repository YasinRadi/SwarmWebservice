...
class DataModel
{
    ...
    static insertModule(res, name, email)
    {
        return data.query('SELECT id FROM account WHERE email = \'' + email + '\'', res);
        //data.query('INSERT INTO module (name, account_id) VALUES (\'' + name + '\', ' + account_id + ')');
    }
